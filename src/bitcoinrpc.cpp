...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Stop BitCrystalSaphir server (and possibly override the detachdb config value).");
    // Shutdown will take long enough that the response should get back
    if (params.size() > 0)
        bitdb.SetDetach(params[0].get_bool());
    StartShutdown();
    return "BitCrystalSaphir server stopping";
}
...
{ //  name                      function                 safemd  unlocked
  //  ------------------------  -----------------------  ------  --------
	...
	{ "getstakegenerate",       &getstakegenerate,       true,   false },
    { "setstakegenerate",       &setstakegenerate,       true,   false },
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: BitCrystalSaphir-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
static string HTTPReply(int nStatus, const string& strMsg, bool keepalive)
{
    if (nStatus == HTTP_UNAUTHORIZED)
        return strprintf("HTTP/1.0 401 Authorization Required\r\n"
            "Date: %s\r\n"
            "Server: BitCrystalSaphir-json-rpc/%s\r\n"
...
void ThreadRPCServer(void* parg)
{
    ...
    RenameThread("BitCrystalSaphir-rpclist");
...
void ThreadRPCServer2(void* parg)
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use bitcrystalsaphird";
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("%s, you must set a rpcpassword in the configuration file:\n %s\n"
              ...
              "for example: alertnotify=echo %%s | mail -s \"BitCrystalSaphir Alert\" admin@foo.com\n"),
                strWhatAmI.c_str(),
...
void ThreadRPCServer3(void* parg)
{
    ...
    RenameThread("BitCrystalSaphir-rpchand");
...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
	...
	if (strMethod == "setstakegenerate"       && n > 0) ConvertTo<bool>(params[0]);
    if (strMethod == "setgenerate"            && n > 1) ConvertTo<int64_t>(params[1]);
	if (strMethod == "setstakegenerate"       && n > 1) ConvertTo<int64_t>(params[1]);
