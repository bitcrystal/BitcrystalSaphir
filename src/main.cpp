...
int nCoinbaseMaturity = 40; // BitCrystalSaphirs Transactions need 1 Confirmation for the Coins to be Spendable
...
const string strMessageMagic = "BitCrystalSaphir Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // BitCrystalSaphirs: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // BitCrystalSaphirs: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// BitCrystalSaphirs: find block wanted by given orphan block
...
int64 GetProofOfStakeReward(int64 nCoinAge, u32int nTime, bool useProofOfBurn)
{
  int64 nRewardCoinYear = CENT;  // creation amount per coin-year
...
static inline CBigNum GetProofOfStakeLimit(u32int nTime, bool useProofOfBurn)
{
...
// BitCrystalSaphirs: find last block index up to pindex
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // BitCrystalSaphirs: target change every block
    // BitCrystalSaphirs: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // BitCrystalSaphirs: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
	...
	{
		int64_t nReward = GetProofOfBurnReward(nFees);
        // Check coinbase reward
        if (vtx[0].GetValueOut() > nReward)
            return DoS(50, error("ConnectBlock() : coinbase reward exceeded (actual=%"PRId64" vs calculated=%"PRId64")",
                   vtx[0].GetValueOut(),
    ...
    {
        // BitCrystalSaphirs: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // BitCrystalSaphirs: track money supply and mint amount info
...
// BitCrystalSaphirs: total coin age spent in transaction, in the unit of coin-days.
...
// BitCrystalSaphirs: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // BitCrystalSaphirs: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // BitCrystalSaphirs: compute stake entropy bit for stake modifier
    ...
    // BitCrystalSaphirs: compute stake modifier
...
bool CBlock::AcceptBlock()
{
	...
	if (IsProofOfBurn() && nHeight > LAST_POB_BLOCK)
        return DoS(100, error("AcceptBlock() : reject proof-of-burn at height %d", nHeight));
	
	if(nHeight <= LAST_POB_BLOCK)
	{
    ...
    if (nBits != GetNextTargetRequired(pindexPrev, IsProofOfStake()))
        return DoS(100, error("AcceptBlock() : incorrect %s", IsProofOfWork() ? "proof-of-work" : IsProofOfStake() ? "proof-of-stake" : "proof-of-burn"));
    ...
    // BitCrystalSaphirs: check pending sync-checkpoint
   ...
   /* CBigNum bnTarget;
    ...
    if(IsProofOfStake())
    {
      ...
      if (!(pprev->IsProofOfStake() && pprev->pprev->IsProofOfStake()))
        return (nBlkTrust + (2 * bnLastBlockTrust / 3)).getuint256();
      ...
      return (nBlkTrust + bnNewTrust).getuint256();
    }
}
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // BitCrystalSaphirs: check proof-of-stake
    ...
    // BitCrystalSaphirs: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // BitCrystalSaphirs: getblocks may not obtain the ancestor block rejected
    ...
    // BitCrystalSaphirs: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BitCrystalSaphir", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // BitCrystalSaphirs: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // BitCrystalSaphirs: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // BitCrystalSaphirs: tell downloading node about the latest block if it's
